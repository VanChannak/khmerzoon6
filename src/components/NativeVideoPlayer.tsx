...
const NativeVideoPlayer = ({ 
  ...
  // State for restricted server selection (mobile-only on web, web-only on native)
  const [showRestrictedOverlay, setShowRestrictedOverlay] = useState<'mobile_only' | 'web_only' | null>(null);
  ...
  // - web_only: only available on web browser (including PWA) - NOT on native apps
  // - mobile_only: STRICTLY native apps only (Capacitor Android/iOS) - NOT PWA or web browser
  const allAvailableSources = useMemo(() => {
    return videoSources.filter(source => {
      ...
      // Web-only: available on web/PWA, NOT on native apps
      if (permission === 'web_only' && !isNativePlatform) return true;
      // Mobile-only: STRICTLY native Capacitor apps only, NOT web/PWA
      if (permission === 'mobile_only' && isNativePlatform) return true;
      return false;
    });
  }, [videoSources, isNativePlatform]);

  const allSourcesWebOnly = useMemo(() => {
    if (isNativePlatform) return false; // Don't show web-only message on native apps
  ...
  const handleQualityChange = useCallback(async (quality: string) => {
    ...
    if (currentServer?.quality_urls && typeof currentServer.quality_urls === 'object') {
      ...
      if (newUrl && videoRef.current) {
        ...
        logNativeDebug('handleQualityChange', 'Switching MP4 quality', { quality, savedTime, wasPlaying });
        
        // Show loading state
        setIsLoading(true);
        
        // Use Shaka Player to load the new MP4 source for better buffering
        try {
          await loadShakaSource(newUrl);
          ...
          if (video) {
            ...
            if (wasPlaying) {
              await video.play().catch((e) => {
                logNativeDebug('handleQualityChange', 'Play failed after quality switch', e);
              });
            }
            
            logNativeDebug('handleQualityChange', 'Quality switch complete', { newTime: video.currentTime });
          }
        } catch (error) {
          logNativeDebug('handleQualityChange', 'Quality switch failed, falling back', error);
          
          // Fallback: direct source change
          if (videoRef.current) {
  ...
  }, [currentServer, sourceType, setShakaQuality, setShakaAutoQuality, loadShakaSource]);
  ...
  return (
    <>
      ...
      >
        ...
        {!isScreenLocked && !accessLoading && !isLocked && showControls && !showRestrictedOverlay && (
          <div className="absolute top-2 right-2 z-[60] flex items-center gap-2 transition-opacity duration-300">
            {/* Server Selector - Only show if there are 2 or more servers (show all sources including mobile-only) */}
            {playerSettings.showServerSelector && videoSources.length >= 2 && (
              <DropdownMenu>
                ...
                <DropdownMenuContent 
                  className="bg-black/90 backdrop-blur-md text-white border border-white/20 shadow-xl min-w-[180px]"
                  align="end"
                  sideOffset={8}
                ...
                >
                  ...
                  {videoSources.map((source, index) => {
                    ...
                    // Check if this source is available on current platform
                    const isMobileOnlyOnWeb = permission === 'mobile_only' && !isNativePlatform;
                    const isWebOnlyOnNative = permission === 'web_only' && isNativePlatform;
                    const isRestricted = isMobileOnlyOnWeb || isWebOnlyOnNative;
                    ...
                    return (
                      <DropdownMenuItem
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          if (isRestricted) {
                            // Show the restricted overlay when clicking on restricted server
                            setShowRestrictedOverlay(isMobileOnlyOnWeb ? 'mobile_only' : 'web_only');
                          } else {
                      ...
                      >
                        ...
                        {/* Show (App) label for mobile-only sources when on web */}
                        {isMobileOnlyOnWeb && (
                          <span className="ml-1 text-xs text-primary">(App)</span>
                        )}
                        {/* Show (Web) label for web-only sources when on native */}
                        {isWebOnlyOnNative && (
                          <span className="ml-1 text-xs text-blue-400">(Web)</span>
                        )}
                        {source.quality && (
        ...
        {/* Restricted Server Overlay - Show when user clicks on mobile-only or web-only server */}
        {showRestrictedOverlay && (
          <div className="absolute inset-0 z-[70]">
            <AppLockOverlay 
              type={showRestrictedOverlay} 
              contentBackdrop={contentBackdrop}
              onClose={() => setShowRestrictedOverlay(null)}
            />
          </div>
        )}
