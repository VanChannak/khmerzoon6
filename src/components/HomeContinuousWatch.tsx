...
const HomeContinuousWatch = () => {
  ...
  const fetchRecentCast = async () => {
    try {
      ...
      if (user) {
        // Get recent watch history with content details for logged-in users
        const {
          data: historyData
        } = await supabase.from('watch_history').select(`
            content_id,
        ...
        if (historyData) {
          contentData = historyData.map(item => item.content).filter(Boolean);
        }
      }

      // For guest users or if no watch history, use popular content
              ...
              minHeight: '179px'
                ...
                <div className="relative h-full">
                  ...
                  scrollBehavior: 'smooth'
                    ...
                    {watchHistory.map(item => <div key={item.id} onClick={() => handleContentClick(item)} className="cursor-pointer group/card border-2 border-border/50 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:border-primary bg-background/20 dark:bg-black/20 backdrop-blur-sm h-full">
