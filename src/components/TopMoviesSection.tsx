...
const TopMoviesSection = ({ className }: TopMoviesSectionProps = {}) => {
  ...
  const fetchCastWithImages = async (tmdbId: number): Promise<CastMember[]> => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${tmdbId}/credits?api_key=5cfa727c2f549c594772a50e10e3f272`
      );
    ...
    const fetchTopContent = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('content')
          .select('id, title, overview, poster_path, backdrop_path, genre, tmdb_id, access_type, recent_episode, content_type')
        ...
        const contentWithCast = await Promise.all(
          (data || []).map(async (item) => {
            if (item.tmdb_id) {
              const cast = await fetchCastWithImages(item.tmdb_id);
              return { ...item, cast };
            }
            return item;
          })
