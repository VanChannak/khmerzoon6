...
const VideoPlayer = ({ 
  ...
  const displayedSources = useMemo(() => {
    ...
    // - web_only: only available on web browser (including PWA)
    // - mobile_only: only available on TRUE native app (Capacitor Android/iOS) - NOT PWA or web browser
    return base.filter(source => {
      const permission = source.permission || 'web_and_mobile';
      if (permission === 'web_and_mobile') return true;
      // Web-only: available on all web platforms (browser + PWA), NOT on native apps
      if (permission === 'web_only' && !isNativePlatform) return true;
      // Mobile-only: STRICTLY native apps only (Capacitor), NOT PWA or web browser
  ...
  // Check if all sources are mobile-only (to show lock message on web/PWA)
  // This will show lock message on web browser AND PWA - only native apps can play
  const allSourcesMobileOnly = useMemo(() => {
    // Only native Capacitor apps can play mobile_only content
  ...
  return (
    ...
    >
      ...
      {(sourceType === "embed" || sourceType === "iframe") && !isLocked && !accessLoading && !allSourcesMobileOnly && !allSourcesWebOnly && !isCurrentServerRestricted && (
        // Only use PreviewOnlyEmbed for Russian platforms on native apps (not PWA/web)
        isNativePlatform && isRussianVideoPlatform(currentServer?.url || '') ? (
          // Russian platforms (VK, OK.ru, Yandex) - render as non-interactive preview on native apps only
          <PreviewOnlyEmbed
          ...
          // Standard interactive iframe for all other cases (web, PWA, or non-Russian platforms)
          <iframe
            ref={iframeRef}
            src={currentServer.url}
            className="w-full h-full"
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="bg-background text-foreground border border-border shadow-xl z-[10000]">
              ...
              {videoSources.map((source) => {
                ...
                const isDisabled = isMobileOnlyOnWeb || isWebOnlyOnNative;
                return (
                  <DropdownMenuItem
                    key={source.id}
                    onClick={() => !isDisabled && handleServerChange(source)}
                    disabled={isDisabled}
                    className={`${isActive ? "bg-primary text-primary-foreground" : isDisabled ? "opacity-60 cursor-not-allowed" : "hover:bg-muted"}`}
                  >
                    <Icon className="h-4 w-4 mr-2" />
                    <span>{source.server_name || source.name || 'Server'}</span>
                    {isMobileOnlyOnWeb && (
                      <span className="ml-2 text-xs text-primary">(App)</span>
                    )}
                    {isWebOnlyOnNative && (
                      <span className="ml-2 text-xs text-blue-500">(Web)</span>
                    )}
                  </DropdownMenuItem>
                );
              })}
      ...
      {sourceType !== "embed" && sourceType !== "iframe" && videoSources.length > 1 && !accessLoading && (
